...
describe 'ExpandedPerson' do
  ...
  context 'when no JSON formatted data is provided' do
    ...
    it 'should be a person' do
      expect(expanded_person).to be_a_kind_of(Conversions::Collections::PeopleHelpers::Person)
    end
  end
  context "when proper JSON formatted data is provided" do
    ...
    subject(:expanded_person) { Conversions::Collections::PeopleHelpers::ExpandedPerson.new(json_data, json_data_expand) }
